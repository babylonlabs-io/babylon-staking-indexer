...
func (_m *DbInterface) SaveBTCDelegationCovenantSignature(ctx context.Context, stakingTxHash string, covenantBtcPkHex string, signatureHex string, stakeExpansionSignatureHex string) error {
	...
	if rf, ok := ret.Get(0).(func(context.Context, string, string, string, string) error); ok {
		r0 = rf(ctx, stakingTxHash, covenantBtcPkHex, signatureHex, stakeExpansionSignatureHex)
	} else {
		r0 = ret.Error(0)
	}
...
// UpdateStakingParamMaxFinalityProviders provides a mock function with given fields: ctx, version, maxFinalityProviders
func (_m *DbInterface) UpdateStakingParamMaxFinalityProviders(ctx context.Context, version uint32, maxFinalityProviders uint32) error {
	ret := _m.Called(ctx, version, maxFinalityProviders)

	if len(ret) == 0 {
		panic("no return value specified for UpdateStakingParamMaxFinalityProviders")
	}
